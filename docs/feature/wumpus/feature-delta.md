...
### Addendum (2026-06-08): re-validation against agent-benchmark research

After this wave shipped, four research summaries were added under `docs/research-summaries/` — **ABC** (`agentic-benchmarks/`), **AI agent benchmarks are broken** (`benchmarks-broken/`), **LLM-Modulo** (`llm-modulo/`), and **Meta-Harness** (`meta-harness/`) — and the design was re-audited against them. Verdict: the substrate still clears the bar and is, if anything, better-positioned than at design time — it owns a *sound* oracle (rare in agentic benchmarking) and a contamination-proof obfuscation metric, and structurally avoids the named failure modes in `[REF] Benchmark Substrate Notes`. The audit surfaced three new caveats and one decision.

- **L18 — Harness effort is a confound across the LLM cells (D/E/F).** NEW; not anticipated in the original design. Meta-Harness (`docs/research-summaries/meta-harness/`) and the Terminal-Bench 2.0 result (a harness change *alone* moved an agent from ~top-30 to ~top-5) show harness quality can dominate model quality. Cells D (MPL), E (LangGraph), and F (LangChain) differ in *both* cage strength *and* harness implementation quality — so a "D beats F" result confounds "stronger cage" with "better-engineered harness," directly threatening the headline claim that *the cage does the work*. The `[REF]` Scaffolding-leak definition covers the leak *surface* but not effort-as-confound. *Impact:* the matrix's core comparison loses internal validity if harness effort varies across D/E/F. *Mitigation:* hold harness effort roughly constant across the LLM cells (shared scaffolding skeleton, equal tuning/token budget per cell), or scope the claim to "cage-shape given comparable harness effort." Candidate harness-feature KPI: a per-cell harness-effort declaration (LOC / tuning iterations / token budget) reported alongside results.

- **L19 — The Yob (classic) win-rate is contaminated by construction; never headline it alone.** Hunt the Wumpus (1973) saturates public training corpora — which is the *premise* of the obfuscation gap (LLM-Modulo, `docs/research-summaries/llm-modulo/`), not a defect. But it means the Yob number conflates retrieval with reasoning; only the **Yob − Mystery gap** isolates reasoning. *Impact:* a Yob-only figure published as a capability number is meaningless and invites the "outcome validity" critique (`docs/research-summaries/benchmarks-broken/`). *Mitigation:* the harness MUST report Yob and Mystery side-by-side with the gap as the headline metric, never a Yob-only capability claim. Strengthens **L12**.

- **L20 — Task/ecological validity is deliberately traded for internal validity.** ABC's "two validities" framing asks whether solving the task *requires* the capability of interest. Winning a toy 1973 game has high *outcome* validity (sound oracle) but arguable *task* validity for long-horizon agency. Wumpus is an instrument / microbenchmark — a wind tunnel, not a road test. *Impact:* over-claiming turns a legitimate mechanism probe into an indefensible capability leaderboard. *Mitigation:* keep all published claims scoped to *mechanism* ("the cage isolates reasoning from retrieval under perfect determinism"; "the cage does the work, given comparable harness effort"), never *capability* ("LLMs are good/bad agents"). The L3/L4 escalation ladder is the bridge toward task validity; gate any capability-flavored claim behind it.

- **Decision — Promote the deferred ABC items from prose to hard KPIs on the harness feature.** L1/L2/L9/L10 and ABC items **T.8** (per-seed solvability), **T.9** (oracle solver), **R.10** (statistical significance), **R.12/R.13** (non-AI + trivial baselines) are currently recorded as harness *responsibilities in prose*. The benchmarks-broken finding is that "report baselines / significance later" is exactly how agentic benchmarks rot. When the harness feature is specced, these MUST become `K`-style testable gates (solvability-filter coverage; baseline-inclusion check; significance-reporting check; the L19 obfuscation-gap-not-Yob headline check) so they cannot be silently dropped — the same way K-1…K-8 gate the engine. Recorded here as a DISCUSS-wave handoff to the (future) harness feature-delta.
...
### Addendum (2026-06-10): re-validation against three agent-disaster field reports

After the 2026-06-08 audit, three more research summaries were added under `docs/research-summaries/` — **Project Vend** ([`project-vend/`](../../research-summaries/project-vend/project-vend.md)), **the Replit database deletion** ([`replit-database/`](../../research-summaries/replit-database/replit-database.md)), and **the OpenClaw email deletion** ([`openclaw-emails/`](../../research-summaries/openclaw-emails/openclaw-emails.md)). Unlike the ABC / benchmarks-broken cluster, these are *incident reports*, not benchmark-methodology papers: they add no validity gates, they name **failure modes the matrix should instrument and harness axes it should vary**. Verdict: the matrix's core comparison (cage strength, cells A–G) already anticipates most of what they describe — but one (context-window compaction evicting a live constraint) is a confound the current design does not isolate, and which the matrix is unusually well-positioned to *measure*. Two new caveats, one positioning note, one flagged probe.

- **L21 — Context-management policy is an un-isolated confound across the LLM cells (D/E/F/G) — and a measurable cage dividend.** NEW. The [OpenClaw incident](../../research-summaries/openclaw-emails/openclaw-emails.md) deleted 200+ emails because a constraint that was *present and being obeyed* was summarized out of the volatile context window during compaction. Wumpus runs are multi-turn (~50 turns); any memory-backed harness (cell G, and cell F with conversation memory) will compact mid-game, while cell D holds the rules as chart topology + guards that **never enter the context window** and therefore cannot be evicted. *Impact:* (a) an unexamined D-vs-G gap silently bundles "stronger cage" with "cage keeps the rules out of the evictable window" — a confound distinct from **L18**'s harness-*effort* confound; (b) more importantly, this turns the harebrain thesis into a *quantity* rather than an assertion — the cage's immunity to compaction is directly measurable here. *Mitigation:* (i) add a **constraint-eviction event** to harness telemetry — a sibling to the **Divergence event** (`definitions.md`): divergence is *claimed state ≠ ledger*; eviction is *the rules the agent was given are no longer in its context*. (ii) Treat **context-pressure** (small vs large context budget, or injected-verbose observations that force compaction) as an *optional escalation rung* aimed specifically at the constraint-in-context cells — **not** a mandatory fourth axis (a full context-policy × cell × contamination × escalation cross is combinatorially prohibitive). (iii) When reporting D vs G, declare each cell's context-management policy alongside L18's effort declaration.

- **L22 — Self-report faithfulness is a distinct, scorable metric (claimed *history*, not just claimed *state*).** NEW. The [Replit agent](../../research-summaries/replit-database/replit-database.md) produced a fluent confession that contained a *false* claim ("the rollback won't work" — it did). The existing **Divergence event** catches claimed *state* ≠ ledger; Replit adds claimed *causality/history* ≠ ledger. *Impact:* a harness that asks an agent to narrate what it did (common in the trusted-narrator cells E/F and the bare-agent cell G) and then trusts that narration imports exactly the error the ledger-as-truth decision (**L12**, ABC T.5) exists to prevent. *Mitigation:* if any cell collects post-hoc agent self-reports, score them against the ledger as a named **self-report-faithfulness** metric; never let a self-report substitute for a structured state diff. Mostly *corroborates* existing decisions — surfaced so the harness wave makes the metric explicit rather than leaving it implicit.

- **Positioning note (strengthens L20).** All three incidents are real long-horizon-agency disasters whose mechanisms — unbounded action surface ([Project Vend](../../research-summaries/project-vend/project-vend.md)), prose-not-guard ([Replit](../../research-summaries/replit-database/replit-database.md)), context eviction ([OpenClaw](../../research-summaries/openclaw-emails/openclaw-emails.md)) — are precisely what the matrix cells differ on. They are the real-world referent **L20** said the toy lacked: the wumpus cells are a wind tunnel for the exact failures that wiped a production database and a personal inbox. This *strengthens* the mechanism-scoped claim ("the cage isolates reasoning from retrieval, and keeps the rules out of the evictable window") without licensing any capability claim. Project Vend additionally maps onto the existing latitude / scaffolding-leak surface (`definitions.md`): identity-by-hallucination is a divergence event about the agent's *own* position — structurally impossible in D (a Manifest lookup), expected in G (a generation).

- **Optional probe (flagged, not committed).** OpenClaw's emergency-stop failure — a remote "stop" lost the priority contest against the in-flight task — suggests a *mid-run HALT-honoring* probe: inject a halt signal mid-game and measure whether each cell obeys (a chart edge reachable from every state in D; a suggestion the agent may ignore in G). Possibly out of scope for a game-playing benchmark; recorded for the harness wave to accept or reject.

These items are **harness-wave handoffs**, not engine gates — the engine cannot enforce context-management policy or self-report scoring in principle. They extend the 2026-06-08 ABC handoff with the failure-mode lens the three field reports supply.
