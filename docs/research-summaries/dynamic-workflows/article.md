...
*By Thariq Shihipar and Sid Bidasaria — members of technical staff at Anthropic, working on Claude Code.*
*Originally published as an X Article on 2026-06-02 ([source](https://x.com/trq212/status/2061907337154367865)).*

> **Capture note.** This is a faithful reproduction of the X Article. A handful of inline hyperlinks could not be recovered from the source and are marked `[link]`. Figures were downloaded from the article and live in `images/`.

![Cover: a harness for every task](images/fig-00-cover.jpg)
...
That said, best practices are still developing! Dynamic workflows often use more tokens, so think carefully about when and how to use them. (Note: this post is also `[link]`.)
...
- "Use a workflow to dig through `[link]` in Slack for the past six months and find recurring root causes where nobody has filed a ticket."
...
Dynamic workflows execute a JavaScript file with a few special functions that help spawn and coordinate agents:

![Special functions available in a dynamic workflow script](images/fig-01-workflow-functions.jpg)
...
## Why workflows: the failure modes they fight
...
But because static workflows need to work for all edge cases, they are usually more generic. With `[link]` and dynamic workflows, Claude is now intelligent enough to write a custom harness tailor-made for your use case.

![Static vs. dynamic workflows](images/fig-02-static-vs-dynamic.png)
...
![Common workflow patterns](images/fig-03-patterns.jpg)
...
![Large refactor / migration workflow](images/fig-04-zig-to-rust.png)

### Large refactors and migrations

`[link]` was rewritten from Zig to Rust using workflows. You can read more about how that was done in `[link]`.
...
![Research workflow](images/fig-05-research.jpg)
...
![Fact-checking workflow](images/fig-06-verification.jpg)
...
![Sorting / tournament workflow](images/fig-07-sorting-tournament.jpg)
...
![Debugging workflow](images/fig-08-debugging.jpg)
...
### Exploring approaches (taste-based work)
...
## When (and when not) to use workflows
...
![Saving a workflow](images/fig-09-saving.jpg)
...
![Sharing a workflow via a skill](images/fig-10-sharing-skill.jpg)
...
*Thariq Shihipar and Sid Bidasaria are members of technical staff at Anthropic, working on Claude Code.*
