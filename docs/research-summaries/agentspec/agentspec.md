...
**Sources.** Wang, H., Poskitt, C. M., & Sun, J. "AgentSpec: Customizable Runtime Enforcement for Safe and Reliable LLM Agents." *Proceedings of the 48th International Conference on Software Engineering (ICSE 2026)*, Rio de Janeiro. arXiv:2503.18666 ([raw PDF in this folder](AgentSpec-Customizable-Runtime-Enforcement-for-Safe-and-Reliable-LLM-Agents-2503.18666.pdf)). Benchmarks referenced: RedCode-Exec (code), SafeAgentBench (embodied), FixDrive over Baidu Apollo (autonomous driving). Related enforcement work the paper contrasts itself with: Nemo (dialogue-level NL constraints), GuardAgent (LLM-interpreted constraints), llama.cpp grammars and LangChain LCEL (syntactic output filters). All diagrams above are original SVGs drawn for this page.
