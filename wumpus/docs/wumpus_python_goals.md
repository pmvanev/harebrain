...
| [Establishing Best Practices for Building Rigorous Agentic Benchmarks (Zhu et al. 2025, arXiv:2507.02825)](../../2507.02825v5.pdf) | The ABC quality framework. The engine substrate is structurally aligned (see `docs/feature/wumpus/feature-delta.md` § `[REF] Benchmark Substrate Notes`); the harness/benchmark-runner feature that consumes this engine's ledgers MUST take ABC as a primary input — especially T.8 (per-seed solvability), T.9 (oracle solver), R.10 (statistical significance), R.12/R.13 (non-AI + trivial-agent baseline reporting), O.b/O.c (substring matching and LLM-as-judge validation). |
