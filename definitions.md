...
## Shots: in-context examples & attempt budgets

> **One word, three axes.** "*N*-shot" is overloaded. (1) The **in-context-learning** axis — how many worked *examples* sit in the prompt (the dominant, academic sense; see the rows below). (2) The **attempt-budget** axis — how many sampled *tries* the agent gets against an oracle (see **pass@k**). (3) The colloquial **interaction-shape** axis — "one-shot" as *a single self-contained prompt that completes the whole task autonomously*, versus an **iterative** multi-turn loop that stages context, requests an action, then refines the output through human review and feedback. When someone says "one-shot," check which they mean: one example, one sampled try, or one turn / no human in the loop. This repo's benchmarks (**SWE-Bench**, **Terminal-Bench**) care about the attempt axis; prompting/**obfuscation gap** discussion cares about the example axis; harness/workflow discussion (the **cage**, **EDD**, **back-prompt**) cares about the interaction axis.

| Term | Definition |
|---|---|
| **Zero-shot** | No worked examples in the prompt — just the instruction and the input. The model relies entirely on pretraining. The phrase comes from *zero-shot learning* in classical ML (classify a class never seen in training); for LLMs it means "no demonstrations." Most production prompting is zero-shot (instructions, not examples). The **obfuscation gap** is most diagnostic here: with no examples to pattern-match, a zero-shot drop on renamed tokens is harder to wave away as retrieval. |
| **One-shot** | *ICL sense:* exactly one worked example (input→output demonstration) before the real input — the minimal nudge toward a format or style. *Attempt sense:* a single sampled attempt with no retry (pass@1; a.k.a. **single-shot**). *Interaction sense (common colloquial usage):* one self-contained prompt that asks the agent to do the **whole task end-to-end, autonomously**, as opposed to an iterative session where you build context, request an action, then refine on review. Disambiguate by context — the three rarely coincide. |
| **One-shot vs. iterative (interaction shape)** | The practitioner's axis: **one-shot** = fire one complete prompt and let the agent run the task to done with no human in the loop; **iterative** = a multi-turn loop of stage-context → act → review → back-prompt → repeat. The iterative loop is a *human-driven* **back-prompt** cycle (cf. **LLM-Modulo**'s Generate-Test-Critique, with the human as the critic). The harebrain reading: a true one-shot delegation leans hardest on the **cage** to keep an unsupervised brain honest (sound critics instead of a human reviewer), where the iterative shape substitutes human judgment each turn. **EDD**'s adversarial-review step is the iterative loop made into a protocol. |
| **Few-shot** | A handful of worked examples (typically 2–~10) in the prompt. The dominant academic term, from the GPT-3 paper *"Language Models are Few-Shot Learners"* (Brown et al. 2020), which framed in-context learning as the headline capability. Cheap behavior-shaping with no weight updates — but the lift is often pattern-matching, which is exactly what the **obfuscation gap** measures. |
| **Multi-shot** | Anthropic's house term for **few-shot**: their prompt-engineering docs title the technique *"Use examples (multishot prompting)."* So yes — "multi-shot" *is* used in regular practice, but mostly within the Anthropic ecosystem; "few-shot" is the broader/academic default. Treat them as synonyms (more-than-one example), with "multi-shot" leaning toward the larger end of the range. |
| **N-shot / k-shot** | The generic parameterized form: *N* worked examples (5-shot, 32-shot, …). Benchmark leaderboards quote it to fix the comparison (MMLU 5-shot vs. 0-shot are different numbers). Rising *N* usually helps up to a point, then plateaus or degrades as the prompt fills. |
| **In-context learning (ICL)** | The umbrella mechanism the shot-count axis describes: the model adapts its behavior from examples *in the prompt* at inference time, with **no weight updates**. Contrast with fine-tuning (changes weights) and with the **cage**'s structural conditioning (changes *what the model is asked*, not what it has seen). The harebrain reading: ICL is brain-side conditioning; it shapes the leaf, not the topology. |
| **pass@k** | The *attempt-budget* metric: probability that **at least one** of *k* sampled attempts is correct. pass@1 is the strict single-attempt score; pass@10 lets the model try ten times and counts a win if any passes. Inflates with *k*, so always read which *k* a benchmark quotes. This is the "shot" that means *tries*, not *examples* — and it only means anything when a **hard critic** (sound verifier) can tell a passing attempt from a failing one, which is why it's well-defined on **SWE-Bench** and slippery on oracle-less benchmarks. |
